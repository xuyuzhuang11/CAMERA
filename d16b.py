...
def get_d16b(model):
    ...
    model = AutoModelForCausalLM.from_pretrained(model, config=config, torch_dtype='auto', trust_remote_code=True)          # load in cpu
    model.seqlen = 2048
    return model


@torch.no_grad()
def d16b_importance(model, dataloader, dev):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        if i >= model.config.first_k_dense_replace:
            module = get_moe_module(layer, 'd16b')
            mei = MEI(module, dev, tp='d16b', batch_size=args.chunk_size, shape=inps.shape)
            # mei.importance = torch.load(args.save + '/importance.pt')[i-model.config.first_k_dense_replace]
            
            def _add_batch():
                def tmp(_, inp, out):
                    mei.add_batch(inp[0].data, out.data)
                return tmp
            handle = module.register_forward_hook(_add_batch())
            for j in range(0, args.nsamples, args.chunk_size):
                outs[j:j+args.chunk_size] = layer(inps[j:j+args.chunk_size], attention_mask=attention_mask, position_ids=position_ids)[0]
            handle.remove()
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'model', type=str,
        help='D16B model to load; pass location of hugginface converted checkpoint.'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--func', type=str, default='imp',
        help='Functions of this script to run. [imp, prune]'
    )
    parser.add_argument(
    ...
    if args.func == 'prune':
        ...
        tick = time.time()
        importances = d16b_importance(model, dataloader, DEV)
