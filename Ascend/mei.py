...
class MEI:
    ...
    def est_importance(self, reduce_ratio=1.0):
        ...
        if self.tp == 'qwen3':
            ...
            self.inp = self.inp.reshape(self.n_samples, self.seqlen, self.d_model)
            return self.importance
        elif self.tp == 'pangu-embedded':
            W_up = torch.stack([self.module.up_proj.weight], dim=0)         # [1, d_ff]
            W_gate = torch.stack([self.module.gate_proj.weight], dim=0)
            W_down = torch.stack([self.module.down_proj.weight], dim=0)
            n_experts = 1
            d_ff = W_up.shape[1]
            
            base_energy = torch.zeros(n_experts*d_ff, device=self.dev, dtype=self.dtype)  # [1 * d_ff]
            exp_l2_square = torch.zeros(n_experts*d_ff, device=self.dev, dtype=self.dtype)  # [1 * d_ff]
            exp_linf_square = torch.zeros(n_experts*d_ff, device=self.dev, dtype=self.dtype)  # [1 * d_ff]
            ...
            for i in range(n_experts):
                ...
                pre_act = torch.matmul(self.inp, up_proj.T)
                gate = torch.matmul(self.inp, gate_proj.T)
                act_all = torch.nn.functional.silu(gate) * pre_act    # [n_tokens, d_ff]
                exp_l2_square[me_start:me_end] += (act_all ** 2).sum(dim=0)  # [d_ff]
                largest_act = torch.max(torch.max(torch.abs(act_all), dim=0)[0] ** 2, exp_linf_square[me_start:me_end])  # [d_ff]
                exp_linf_square[me_start:me_end] = largest_act  # [d_ff]
            
            exp_computed = (1 - reduce_ratio) * exp_l2_square + reduce_ratio * exp_linf_square
            self.importance = base_energy * exp_computed        # [1*d_ff]
            self.inp = self.inp.reshape(self.n_samples, self.seqlen, self.d_model)
            return self.importance
        else:
            raise NotImplementedError(f"Unknown model type: {self.tp}")

    def make_pruned_module(self, ratio, is_physical=False):
        # make pruned_module
        if self.tp == 'qwen3':
            ...
            if is_physical is False:
                n_experts = 1
                d_ff = self.module.up_proj.weight.shape[0]
                n_me = n_experts * d_ff
                n_prune = int(n_me * ratio)
                _, indices = torch.topk(self.importance, k=n_prune, largest=False)
                expert_ids = indices // d_ff    # assert equal to 0, because openpangu-7B is dense model
                assert sum(expert_ids) == 0, "expert numbers is not 1!"
                channel_ids = indices % d_ff
                for c in channel_ids.tolist():
                    self.module.up_proj.weight.data[c, :] = 0.0
                    self.module.gate_proj.weight.data[c, :] = 0.0
                    self.module.down_proj.weight.data[:, c] = 0.0
            elif is_physical is True:
                d_ff = self.module.up_proj.weight.shape[0]      # Original intermediate size
                d_model = self.module.up_proj.weight.shape[1]   # Original hidden size
                
                # Number to prune
                n_prune = int(d_ff * ratio)
                n_keep = math.ceil((d_ff - n_prune) / 64) * 64
                print(f"[INFO] n_keep: {d_ff} --> {n_keep}")
                
                # Get indices
                _, kept_indices = torch.topk(self.importance, k=n_keep, largest=True)
                kept_indices, _ = torch.sort(kept_indices)
                
                # Slicing
                # shape of up_proj/gate_proj is [d_ff, d_model], keep the 'row'
                new_up_weight = self.module.up_proj.weight.data[kept_indices, :].clone()
                new_gate_weight = self.module.gate_proj.weight.data[kept_indices, :].clone()
                
                # shape of down_proj is [d_model, d_ff], keep the 'column'
                new_down_weight = self.module.down_proj.weight.data[:, kept_indices].clone()
                
                # Bias
                new_up_bias = None
                if self.module.up_proj.bias is not None:
                    new_up_bias = self.module.up_proj.bias.data[kept_indices].clone()
                new_gate_bias = None
                if self.module.gate_proj.bias is not None:
                    new_gate_bias = self.module.gate_proj.bias.data[kept_indices].clone()
                new_down_bias = None
                if self.module.down_proj.bias is not None:
                    new_down_bias = self.module.down_proj.bias.data.clone()
                
                # Replace Modules
                self.module.up_proj = nn.Linear(d_model, n_keep, bias=(new_up_bias is not None), device=self.dev, dtype=self.dtype)
                self.module.gate_proj = nn.Linear(d_model, n_keep, bias=(new_gate_bias is not None), device=self.dev, dtype=self.dtype)
                self.module.down_proj = nn.Linear(n_keep, d_model, bias=(new_down_bias is not None), device=self.dev, dtype=self.dtype)
                
                self.module.up_proj.weight.data = new_up_weight
                self.module.gate_proj.weight.data = new_gate_weight
                self.module.down_proj.weight.data = new_down_weight
                
                if new_up_bias is not None:
                    self.module.up_proj.bias.data = new_up_bias
                if new_gate_bias is not None:
                    self.module.gate_proj.bias.data = new_gate_bias
                if new_down_bias is not None:
                    self.module.down_proj.bias.data = new_down_bias
            else:
                print("is_physical is not set correctly!")
        else:
            raise NotImplementedError(f"Unknown model type: {self.tp}")
