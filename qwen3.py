...
def get_qwen3(model):
    ...
    model = Qwen3MoeForCausalLM.from_pretrained(model, torch_dtype='auto')          # load in cpu
    model.seqlen = 2048
    return model


@torch.no_grad()
def qwen3_importance(model, dataloader, dev):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        module = get_moe_module(layer, 'qwen3')
        mei = MEI(module, dev, tp='qwen3', batch_size=args.chunk_size, shape=inps.shape)
...
if __name__ == '__main__':
    ...
    if args.func == 'prune':
        ...
        tick = time.time()
        importances = qwen3_importance(model, dataloader, DEV)
