...
def get_qwen2(model):
    ...
    model = Qwen2MoeForCausalLM.from_pretrained(model, torch_dtype='auto')          # load in cpu
    model.seqlen = 2048
    return model


@torch.no_grad()
def qwen2_importance(model, dataloader, dev):
    print('Starting ...')
    ...
    for i in range(len(layers)):
        ...
        module = get_moe_module(layer, 'qwen2')
        mei = MEI(module, dev, tp='qwen2', batch_size=args.chunk_size, shape=inps.shape)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'model', type=str,
        help='Qwen3 model to load; pass location of hugginface converted checkpoint.'
    )
    parser.add_argument(
    ...
    if args.func == 'prune':
        ...
        tick = time.time()
        importances = qwen2_importance(model, dataloader, DEV)
